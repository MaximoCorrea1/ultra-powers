...
(HAVE_GIT_BASH ? describe : describe.skip)("Bug E + D: path-form independence + invoke banner", () => {
  ...
  test("Bug D: the banner reinforces INVOKE (not just state routing) and carries the resolvable FLOW.md path", () => {
    ...
    // The banner explicitly forces the FLOW.md read (the measured lever, 38%->100%),
...
d("flowy-inject.sh", () => {
  ...
  // ritual is intentionally dropped (founder: minimal verbosity; the science
  // shows terse+read == verbose for adherence).
  // -------------------------------------------------------------------------
  test("terse banner: forces the FLOW.md read + invoke before acting, no YES/NO ritual", () => {
    ...
    // Then route/invoke, before any other tool — not just "state routing".
    expect(r.stdout).toMatch(/invoke/i);
    expect(r.stdout).toMatch(/before any other tool/i);
    // The YES/NO commitment ritual is gone (terse).
    expect(r.stdout).not.toContain("YES,<reason>");
    expect(r.stdout).not.toContain("commit each candidate skill");
    // Terseness — a single active flow is EXACTLY one banner line.
  ...
  // -------------------------------------------------------------------------
  // DRIVER 3 (ADR-030): a plugin flow this plugin does NOT ship (no flows/<name>/
  ...
  test("flow active but FLOW.md unresolvable → WARNING line, exit 0 (NOT 2)", () => {
    const dirs = makeDirs();
    // ADR-030: owned-but-broken (dir exists, FLOW.md missing) so it still warns.
  ...
  test("flowRef with shell metachars and NO valid name → corrupt, exit 0", () => {
    const dirs = makeDirs();
    // ADR-030: owned-but-broken so the corrupt warning still fires.
    mkdirSync(join(dirs.pluginRootWin, "flows", "ghost-flow"), { recursive: true });
    writeState(dirs, "A", {
      schema: "flowy-state-v1",
      sessionId: "A",
  ...
  test("corrupt flow name with a dot → exactly one warning line, name intact", () => {
    const dirs = makeDirs();
    // ADR-030: owned-but-broken (dir exists) so the corrupt warning still fires.
  ...
  test("mixed live + corrupt flows → banner for live, warning for corrupt, both in same run, exit 0", () => {
    const dirs = makeDirs();
    writeFlowMd(dirs, "flows/superpowers-flow/FLOW.md");
    // ADR-030: ghost-flow owned-but-broken (dir exists) so it still warns.
    mkdirSync(join(dirs.pluginRootWin, "flows", "ghost-flow"), { recursive: true });
    writeState(dirs, "A", {
      schema: "flowy-state-v1",
      sessionId: "A",
